...
BinaryHeap.prototype.insert = function(node) {
  var storage = this._heap;
  if (storage.length) {
    parentIndex = Math.floor((storage.length - 1) / 2));
    if (storage[parentIndex] < node) {
      this._heap.push(node);
    } else {
      var holder = storage[parentIndex];
      this._heap[parentIndex] = node;
      this._heap[storage.length] = holder;
    }
  } else {
    this._heap.push(node);
  }
...
BinaryHeap.prototype.swap = function(node, nodeIndex, parentIndex) {
  var storage = this._heap;
  if (storage[parentIndex] < node) {
    return;
  }
  var holder = storage[parentIndex];
  var newParentIndex = Math.floor((parentIndex - 1) / 2));
  this._heap[parentIndex] = node;
  this._heap[nodeIndex] = holder;
  this.swap(node, parentIndex, newParentIndex);
}
